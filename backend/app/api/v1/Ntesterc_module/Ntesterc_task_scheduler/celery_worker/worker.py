...
from app.core.logger import InterceptHandler, logger
...
def create_celery():
    ...
    _celery_: Celery = Celery("fastapiwebadmin-celery-worker", task_cls=ContextTask)
...
# Celery 归属 Ntesterc_task_scheduler；CLI 入口见 backend/celery_app.py
# worker windows（单线程）:
#   celery -A celery_app.celery worker --pool=solo -l INFO
# worker linux:
#   celery -A celery_app.celery worker --pool=solo -c 10 -l INFO
# beat:
