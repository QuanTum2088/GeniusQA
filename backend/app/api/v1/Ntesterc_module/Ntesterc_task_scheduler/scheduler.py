...
def get_scheduler() -> AsyncIOScheduler:
    ...
    if _scheduler is None:
        ...
        # 不使用 ProcessPoolExecutor：Windows 上退出时易卡住，且当前业务未使用
...
def shutdown_scheduler() -> None:
    ...
    if scheduler.running:
        # wait=False：不阻塞等待线程池任务，避免 Ctrl+C 卡死
        scheduler.shutdown(wait=False)
    # 释放 JobStore 同步引擎连接
    try:
        store = scheduler._jobstores.get("sqlalchemy")
...
def _write_history(
    *,
    ...
    from app.api.v1.Ntesterc_module.Ntesterc_task_scheduler.model import TaskExecutionHistoryModel
