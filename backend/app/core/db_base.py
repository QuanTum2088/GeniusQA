...
# @author: Rebort
"""
SQLAlchemy 声明式 ORM Base（全局唯一）。
业务模型继承本 Base；带混入的企业模型见 base_model.BaseModel。
"""
...
from app.infra.db.sqlalchemy import async_transaction
...
class Base:
    ...
    async def handle_params(cls, params: typing.Any) -> typing.Any:
        ...
        if isinstance(params, dict):
            ...
            # 自动设置创建人和更新人
            # 注意：在新架构中，created_by 和 updated_by 应该在业务层显式设置
            # 这里保留兼容性，但不再自动获取当前用户
            try:
                # 如果参数中已经包含用户ID，使用它们
                # 否则跳过自动设置，由业务层负责
