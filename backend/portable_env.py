...
# -*- coding: utf-8 -*-
"""
Portable deployment helper: locate config.yaml and map its contents to
environment variables expected by the backend (UVICORN_HOST, UVICORN_PORT,
MYSQL_DATABASE_URI, REDIS_URI, etc.).
"""
...
def _apply_mapping(root: Path, data: Dict[str, Any]) -> None:
    ...
    # CAPTCHA_ENABLE
    if isinstance(sec, dict) and "captcha_enable" in sec:
        os.environ["CAPTCHA_ENABLE"] = "true" if sec["captcha_enable"] else "false"
