...
- **GeniusQA_tools.py** - 测试用例工具，提供本地测试用例管理功能
...
#### GeniusQA Tools (GeniusQA_tools.py) ✨ 部署直接使用，简单快捷

- **服务端口**: 8006
- **后端地址**: `GeniusQA_BACKEND_URL`（默认：http://backend:8018）
- **API密钥**: `GeniusQA_API_KEY`（默认：GeniusQA-default-mcp-key-2025）
...
**⚠️ 生产环境安全提示：**
- 系统默认API Key（GeniusQA-default-mcp-key-2025）
- 开发环境可直接使用
...
```json
{
  "mcpServers": {
    ...
    "testauto-tools": {
      "command": "python",
      "args": ["path/to/GeniusQA_tools.py"],
      "env": {}
    }
  }
}
