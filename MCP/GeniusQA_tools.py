...
base_url = os.getenv("GeniusQA_BACKEND_URL", "http://127.0.0.1:8018")

# 从环境变量读取 API Key
# 请在 .env 文件或环境变量中设置 GeniusQA_API_KEY
api_key = os.getenv("GeniusQA_API_KEY", "")
...
@mcp.tool(description="获取GeniusQA平台项目的名称和对应id")
def get_project_name_and_id() -> str:
    """获取GeniusQA平台项目的名称和对应id"""
    ...
    try:
        ...
        error_info = {
            ...
            "url": url,
            "base_url": base_url,
            "suggestion": "请检查后端服务是否启动，或检查 GeniusQA_BACKEND_URL 环境变量配置"
        }
        return json.dumps(error_info, indent=4, ensure_ascii=False)
...
@mcp.tool(description="根据GeniusQA平台项目id去获取模块及id")
def module_to_which_it_belongs(project_id: int) -> str:
    """根据GeniusQA平台项目id去获取模块及id"""
...
@mcp.tool(description="获取GeniusQA平台用例等级")
...
@mcp.tool(description="获取GeniusQA平台用例名称和对应id")
...
@mcp.tool(description="获取GeniusQA平台用例详情")
...
@mcp.tool(description="GeniusQA平台保存操作截图到对应用例中")
...
@mcp.tool(description='保存GeniusQA平台功能测试用例')
...
@mcp.tool(description='编辑GeniusQA平台功能测试用例')
