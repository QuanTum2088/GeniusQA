...
**基于 FastAPI + Vue3 AI Agent全栈测试管理平台**

一个集成AI Agent、API测试、UI自动化、数据工厂、评审管理等功能的企业级测试平台
...
GeniusQA AI 全栈测试平台是一个面向企业级应用的现代化Agent测试管理系统，采用前后端完全分离的架构设计，集成AIAgent测试、API接口测试、UI自动化测试、数据工厂、评审管理等多个核心功能模块。

**核心特性：**
- 🤖 AI Agent测试：集成大语言模型，Agent生成测试用例和执行测试
- 🔌 API接口测试：完整的API测试生命周期管理
- 🖥️ UI自动化测试：支持Web UI自动化测试执行和管理
- 🏭 数据工厂：强大的测试数据生成和管理工具
- 📋 评审管理：完善的测试用例评审流程
- 📊 Agent报告：可视化测试报告和数据分析
- 🔐 完整的 RBAC 权限管理体系
...
- **边界值分析**：Agent识别边界条件和异常场景
- **用例优化**：根据历史数据优化用例覆盖率

#### 3. Agent执行引擎
- **自适应执行**：根据执行结果动态调整测试策略
- **异常处理**：Agent识别和处理执行过程中的异常
...
#### 1. 元素管理系统
- **Agent定位**：多种元素定位策略，提高稳定性
- **元素库管理**：统一管理页面元素，支持复用
- **动态等待**：Agent等待机制，处理页面加载时序
...
# 编辑 .env 文件，修改数据库连接信息(注意，其他数据库连接配置不使用默认要改为注释）
# MySQL 数据库配置
DB_HOST=localhost
DB_PORT=3306
DB_USER=root
...
### AIAgent对话
![AIAgent对话](static/img/aise.png)

### AIAgent浏览器
![AIAgent浏览器](static/img/aec.png)
...
**GeniusQA AI 全栈测试平台 - 让测试更Agent，让质量更可靠**
