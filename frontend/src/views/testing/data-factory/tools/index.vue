<template>
	<div class="data-factory-tools-container">
		<!-- 页面头部 -->
...
const getCategoryColor = (category: string): string => {
	const colorMap: Record<string, string> = {
		'test_data': 'linear-gradient(135deg, #667eea, #a24b4bff)', // 紫蓝渐变
	...
	console.log(`Tools page - Category: ${category}, Color: ${color}`); // 调试信息
	return color;
};
...
const handleToolExecute = (result: any) => {
	...
	// 可以在这里处理执行结果，比如显示结果对话框
};

// 处理批量生成
const handleBatchGenerate = (result: any) => {
	ElMessage.success(`批量生成成功，共生成 ${result.count} 条数据`);
};
