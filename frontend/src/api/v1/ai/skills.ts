...
export function useSkillsApi() {
  return {
    list: (projectId: number, params?: Record<string, any>) =>
      request({ 
        url: `/v1/skills/projects/${projectId}`, 
        method: 'GET', 
        params 
      }),
    ...
    update: (projectId: number, skillId: number, data: any) =>
      request({ 
        url: `/v1/skills/projects/${projectId}/${skillId}`, 
        method: 'PUT', 
        data 
      }),
    ...
    content: (projectId: number, skillId: number) =>
      request({ 
        url: `/v1/skills/projects/${projectId}/${skillId}/content`, 
        method: 'GET' 
      }),
    manifest: (projectId: number, skillId: number) =>
      request({ 
        url: `/v1/skills/projects/${projectId}/${skillId}/manifest`, 
        method: 'GET' 
      }),
    execute: (projectId: number, skillId: number, data?: { arguments?: Record<string, any>; session_id?: string }) =>
      request({ 
        url: `/v1/skills/projects/${projectId}/${skillId}/execute`, 
        method: 'POST', 
        data: data || {} 
      }),
    executeActionAsync: (
    ...
    job: (projectId: number, jobId: number) =>
      request({ 
        url: `/v1/skills/projects/${projectId}/jobs/${jobId}`, 
        method: 'GET' 
      }),
    jobArtifacts: (projectId: number, jobId: number) =>
      request({ 
        url: `/v1/skills/projects/${projectId}/jobs/${jobId}/artifacts`, 
        method: 'GET' 
      }),
    ...
    importGit: (projectId: number, data: any) =>
      request({ 
        url: `/v1/skills/projects/${projectId}/import/git`, 
        method: 'POST', 
        data 
      }),
